 ...
 * 色泽平衡滤镜，均匀色泽效果
 */
class GPUImageColorBalanceFilter : GPUImageFilter(
    NO_FILTER_VERTEX_SHADER,
    GPU_IMAGE_COLOR_BALANCE_FRAGMENT_SHADER
) {
    private var mShadowsLocation = 0
    private var mMidtonesLocation = 0
    private var mHighlightsLocation = 0
    private var mPreserveLuminosityLocation = 0
    private var showdows: FloatArray = floatArrayOf(0.0f, 0.0f, 0.0f)
    private var midtones: FloatArray = floatArrayOf(0.0f, 0.0f, 0.0f)
    private var highlights: FloatArray = floatArrayOf(0.0f, 0.0f, 0.0f)
    private var preserveLuminosity: Boolean = true

    override fun onInit() {
        super.onInit()
        mShadowsLocation = GLES30.glGetUniformLocation(program, "shadowsShift")
        mMidtonesLocation = GLES30.glGetUniformLocation(program, "midtonesShift")
        mHighlightsLocation = GLES30.glGetUniformLocation(program, "highlightsShift")
        mPreserveLuminosityLocation = GLES30.glGetUniformLocation(program, "preserveLuminosity")
    }
    ...
    fun setPreserveLuminosity(preserveLuminosity: Boolean) {
        ...
        setInteger(mPreserveLuminosityLocation, if (preserveLuminosity) 1 else 0)
    }

    companion object {
        const val GPU_IMAGE_COLOR_BALANCE_FRAGMENT_SHADER = "" +
